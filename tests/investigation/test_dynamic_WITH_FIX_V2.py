#!/usr/bin/env python3
"""
ДИНАМИЧЕСКИЙ ТЕСТ С EVENT-BASED VERIFICATION V2 (УЛУЧШЕННОЕ РЕШЕНИЕ)
FIX:
1. Delay 3s после WebSocket connect (стабилизация)
2. Увеличенный timeout для первых подписок (20s)
3. Агрессивный retry с уменьшающимся timeout
"""
...
class DynamicTesterWithFixV2:
    """Test with Event-based verification FIX V2"""
    ...
    async def subscribe_with_verification(self, symbol: str, timeout: float = None, is_initial: bool = False) -> bool:
        """
        Subscribe with EVENT-BASED verification (NON-BLOCKING!)

        FIX V2: Адаптивный timeout
        - Первые подписки: 20s (WebSocket нужно время для стабилизации)
        - Обычные подписки: 10s
        - Retry: 5s
        ...
        # Адаптивный timeout
        if timeout is None:
            if is_initial or self.total_subscriptions < 10:
                timeout = 20.0  # Первые подписки - увеличенный timeout
            else:
                timeout = 10.0  # Обычный timeout
        ...
        try:
            ...
            try:
                ...
                self.log(f"   ✅ VERIFIED: {symbol} (real data in {timeout:.0f}s)")
                return True
    ...
    async def retry_silent_fail(self, symbol: str) -> bool:
        ...
        FIX V2: Более короткий timeout для retry (5s)
        """
        self.log(f"   🔄 RETRY: {symbol}")
        success = await self.subscribe_with_verification(symbol, timeout=5.0, is_initial=False)
        if success:
    ...
    async def run(self):
        """Run dynamic test WITH FIX V2"""
        self.log(f"\n{'='*80}")
        self.log(f"DYNAMIC TEST WITH EVENT-BASED VERIFICATION FIX V2")
        self.log(f"{'='*80}")
        ...
        self.log(f"🔧 FIX V2:")
        self.log(f"   - WebSocket stabilization delay: 3s")
        self.log(f"   - Initial subscriptions timeout: 20s")
        self.log(f"   - Normal timeout: 10s, Retry: 5s")
        self.log(f"{'='*80}\n")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(self.ws_url) as ws:
                ...
                # FIX V2: Wait 3 seconds for WebSocket stabilization
                self.log(f"⏳ Waiting 3s for WebSocket stabilization...")
                await asyncio.sleep(3.0)

                # Initial subscriptions with verification
                self.log(f"\n📥 Initial subscriptions (10 symbols with 20s timeout)...")
                for i in range(10):
                    if self.pending_symbols:
                        symbol = self.pending_symbols.pop(0)
                        self.initial_subscriptions_count += 1
                        await self.subscribe_with_verification(symbol, is_initial=True)
                        await asyncio.sleep(0.1)

                self.log(f"✅ Initial: {self.total_verified}/{self.initial_subscriptions_count} verified\n")
