...
class SmartTrailingStopManager:
    ...
    async def _update_stop_order(self, ts: TrailingStopInstance) -> bool:
        ...
        try:
            ...
            # ============================================================
            # FIX AIAUSDT: SAFETY BUFFER CHECK (Solution 1)
            # ============================================================
            # Prevent Binance -2021 error: "Order would immediately trigger"
            #
            # Problem: When price is too close to SL level, Binance rejects the order
            # to prevent instant triggering. This is especially problematic for SHORT
            # positions during volatile markets.
            #
            # Solution: Add 0.5% safety buffer - skip update if price too close to SL
            #
            # References:
            # - Investigation: AIAUSDT_TS_FAILURE_INVESTIGATION.md
            # - Binance API Error -2021 documentation
            
            SAFETY_BUFFER_PERCENT = Decimal('0.5')  # 0.5% buffer
            
            if ts.side == 'short':
                # For SHORT: SL is ABOVE price
                # Skip if current_price >= (sl_price * (1 - buffer))
                # Example: SL=$0.3594, buffer=0.5% → skip if price >= $0.3576
                safety_threshold = sl_price * (Decimal('1') - SAFETY_BUFFER_PERCENT / Decimal('100'))
                
                if current_price >= safety_threshold:
                    logger.warning(
                        f"⚠️ {ts.symbol}: SL update SKIPPED - price too close to SL "
                        f"(current: {current_price:.8f}, sl: {sl_price:.8f}, "
                        f"threshold: {safety_threshold:.8f}, buffer: {SAFETY_BUFFER_PERCENT}%). "
                        f"Will retry when price moves away. "
                        f"[SEARCH: ts_update_skipped_price_too_close_{ts.symbol}]"
                    )
                    
                    # Log skipped update event
                    event_logger = get_event_logger()
                    if event_logger:
                        await event_logger.log_event(
                            EventType.WARNING_RAISED,
                            {
                                'warning_type': 'ts_update_skipped_price_too_close',
                                'symbol': ts.symbol,
                                'side': ts.side,
                                'current_price': float(current_price),
                                'proposed_sl': float(sl_price),
                                'safety_threshold': float(safety_threshold),
                                'buffer_percent': float(SAFETY_BUFFER_PERCENT),
                                'message': 'Skipped SL update to prevent Binance -2021 error'
                            },
                            symbol=ts.symbol,
                            exchange=self.exchange_name,
                            severity='WARNING'
                    ...
                    return False  # Skip update, will retry on next price update
                    
            elif ts.side == 'long':
                # For LONG: SL is BELOW price
                # Skip if current_price <= (sl_price * (1 + buffer))
                # Example: SL=$100, buffer=0.5% → skip if price <= $100.50
                safety_threshold = sl_price * (Decimal('1') + SAFETY_BUFFER_PERCENT / Decimal('100'))
            ...
            # Safety check passed - price is far enough from SL
            logger.debug(
                f"✅ {ts.symbol}: Safety buffer check PASSED - "
                f"price {current_price:.8f} is safe distance from SL {sl_price:.8f}"
            )
