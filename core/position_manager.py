...
class PositionManager:
    ...
    def __init__(self,
        ...
        # Locks for trailing stop operations
        self.trailing_stop_locks: Dict[str, asyncio.Lock] = {}
    ...
    async def load_positions_from_db(self):
        ...
        try:
            ...
            for pos in positions:
                ...
                # DEBUG: Log each loaded position
                logger.debug(f"   ✅ Loaded: {pos['symbol']} on {pos['exchange']} (id={pos['id']})")
            ...
            # DEBUG: Log full list of loaded symbols
            loaded_symbols = sorted(list(self.positions.keys()))
            logger.debug(f"   Loaded symbols: {loaded_symbols}")
                ...
                if symbol not in self.positions or self.positions[symbol].exchange != exchange_name:
                    ...
                    if db_position:
                        ...
                        position_state = PositionState(
                            ...
                            has_trailing_stop=db_position.get('has_trailing_stop', True),
    ...
    async def open_position(self, request: PositionRequest) -> Optional[PositionState]:
        ...
        try:
            ...
            # 4. Validate entry price
            if not request.entry_price or request.entry_price <= 0:
                logger.error(f"❌ Invalid entry price in request: {request.entry_price} for {symbol}")
    ...
    async def _position_exists(self, symbol: str, exchange: str) -> bool:
        ...
        async with self.check_locks[lock_key]:
            # DEBUG: Log entry
            logger.debug(f"🔍 _position_exists(symbol='{symbol}', exchange='{exchange}')")

            # Check local tracking - must verify BOTH symbol AND exchange
            logger.debug(f"   Step 1/3: Checking cache...")
            exchange_lower = exchange.lower()
            for pos_symbol, position in self.positions.items():
                if pos_symbol == symbol and position.exchange.lower() == exchange_lower:
                    logger.debug(f"   ✅ Found in cache: {symbol} on {position.exchange}")
                    return True
            logger.debug(f"   ❌ Not in cache")

            # Check database
            logger.debug(f"   Step 2/3: Checking database...")
            db_position = await self.repository.get_open_position(symbol, exchange)
            if db_position:
                logger.debug(f"   ✅ Found in DB: position_id={db_position.get('id')}")
                return True
            logger.debug(f"   ❌ Not in DB")

            # Check exchange
            logger.debug(f"   Step 3/3: Checking exchange API...")
            ...
            if exchange_obj:
                ...
                for pos in positions:
                    if normalize_symbol(pos.get('symbol')) == normalized_symbol:
                        contracts = float(pos.get('contracts') or 0)
                        if abs(contracts) > 0:
                            logger.debug(f"   ✅ Found on exchange: {pos.get('symbol')} with {contracts} contracts")
                            return True
                logger.debug(f"   ❌ Not on exchange")
            else:
                logger.debug(f"   ❌ Exchange object not found")

            logger.debug(f"   Final result: FALSE (not found anywhere)")
            return False
    ...
    async def has_open_position(self, symbol: str, exchange: str = None) -> bool:
        ...
        """
        # DEBUG: Log entry
        logger.debug(f"🔍 has_open_position(symbol='{symbol}', exchange='{exchange}')")
        logger.debug(f"   Cache size: {len(self.positions)} positions")
        ...
        if exchange:
            ...
            logger.debug(f"   ❌ Not in cache, checking DB/exchange via _position_exists()...")
    ...
    async def _on_position_update(self, data: Dict):
        ...
        # Mark price events don't have this field
        ...
        async with self.position_update_locks[symbol]:
            ...
            if trailing_lock_key not in self.trailing_stop_locks:
                self.trailing_stop_locks[trailing_lock_key] = asyncio.Lock()

            async with self.trailing_stop_locks[trailing_lock_key]:
                trailing_manager = self.trailing_managers.get(position.exchange)
